...
namespace Avalonia.Wayland
{
    internal sealed class WlDataObject : IDataObject, IDisposable, WlDataOffer.IEvents
    {
        ...
        public bool Contains(string dataFormat)
        {
            ...
            {
                return MimeTypes.Contains(Wayland.MimeTypes.Text) || MimeTypes.Contains(Wayland.MimeTypes.TextUtf8);                
            }
            else if (dataFormat == DataFormats.FileNames)
            {
                return MimeTypes.Contains(Wayland.MimeTypes.UriList);
            }
            return MimeTypes.Contains(dataFormat);
        }
